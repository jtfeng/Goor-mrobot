...
import cn.muye.base.service.ReceiveService;
...
public class ReceiveServiceImp implements ReceiveService, ApplicationContextAware {
    ...
    public void analysis(Client client, byte[] content){
        logger.info("client====" + client + ",content====" + content);
        try {
            ...
            //此处为打印日志，请勿删除
            logger.info("jsonObject====" + jsonObject + ",messageAll====" + messageAll + ",messageInfo====" + messageInfo + ",messageInfoDecode====" + messageInfoDecode + ",messageInfoJson====" + messageInfoJson);
    ...
    public void navigation(Client client, MessageInfo messageInfo) throws Exception{
        logger.info("messageType====" + messageInfo.getMessageType());
    ...
    private boolean sendMessageSave(MessageInfo messageInfo) throws Exception{
        ...
        if(!MessageType.REPLY.equals(messageInfo.getMessageType())){
            ...
            List<ReceiveMessage> messages = receiveMessageService.listByIdAndSenderId(message);//查询当前发送者有没有发送此记录
            if(messages.size() <= 0){
                //TODO 直接回执，再存库
